...
IMPLICIT NONE
  ...
  INTEGER:: IN=1, OUT=2, DBG=3, GNU=4, SS=7   ! unit assignments
  INTEGER,PARAMETER:: XLSFormat=1,SeligFormat=2
...
!-------------------------------------------------------------------------------
  ...
  IF(OUT==DBG) THEN
    WRITE(*,*) "Spread Sheet file created."
  ELSE
    WRITE(*,*) "Out, gnu, dbg, and spread sheet files created."
  END IF
  WRITE(*,*) FAREWELL
  STOP
...
USE NACAauxilary 
  ...
  REAL:: SSonly = 0   ! generate only a Spread Sheet file
  INTEGER:: SSformat = 0 ! Spread Sheet format
                      ! = 0 CSV
                      ! = 1 XLS 
                      ! = 2 Selig
  REAL:: Rotation = 0 ! rotate airfoil
  INTEGER:: Center = 0! center airfoil
  ...
  NAMELIST /NACA/ a, camber, chord, cl, cmax, dencode,leIndex,name, &
    profile, toc, xmaxc,xmaxt,xorigin,yorigin, xTable, SSonly, &
    SSformat, Rotation, Center
...
!-------------------------------------------------------------------------------
  ...
  ELSE
    OPEN(UNIT=OUT, FILE='/dev/null', ACTION='WRITE', IOSTAT=errCode)
    IF(errCode/=0) THEN
      OPEN(UNIT=OUT, FILE='NUL', ACTION='WRITE')
    END IF
    DBG=OUT
    GNU=OUT
  END IF
  SELECT CASE(SSformat)
    CASE(XLSFormat)
      OPEN(UNIT=SS, FILE=(TRIM(name)//'.xls'), STATUS='REPLACE', ACTION='WRITE')
    CASE(SeligFormat)
      Open(UNIT=SS, FILE=(TRIM(name)//'.dat'), STATUS='REPLACE', ACTION='WRITE')
    CASE DEFAULT
      OPEN(UNIT=SS, FILE=(TRIM(name)//'.csv'), STATUS='REPLACE', ACTION='WRITE')
  END SELECT
...
!!!  WRITE(OUT,'(A,T40,A)') TITLE1,dateTimeStr
  ...
  !DO k=1,SIZE(x)
  !  REAL:: ru =SQRT(EXP(x(k), 2.0)+EXP(yu(k), 2.0))
  !  REAL:: rl =SQRT(EXP(x(k), 2.0)+EXP(yl(k), 2.0))
  !  REAL:: t =ATAN2(yu(k), x(k))

  !  x = ru*COS(Rotation+t)
  !  yu = ru*SIN(Rotation+t)
  !  yl = rl*SIN(Rotation+t)
  !  
  !END DO
  IF (Center==1) THEN
      x = x-.5
...
xmlns:html="http://www.w3.org/TR/REC-html40">&
 <Worksheet ss:Name="Sheet1"><Table>'
      CALL PrintXLS(SS, x*chord, yu*chord, yl*chord)
      WRITE(SS, *) '</Table></Worksheet></Workbook>'
    CASE(SeligFormat)
      WRITE(SS,*) name
      CALL PrintSelig(SS, x*chord, yu*chord, yl*chord)
    CASE DEFAULT
      CALL PrintCSV(SS, x*chord,yu*chord,yl*chord)
  END SELECT
...
SUBROUTINE PrintCSV(efu, x, yu, yl)
  INTEGER,INTENT(IN):: efu
  REAL,INTENT(IN),DIMENSION(:):: x,yu,yl
  165 FORMAT(F0.6,',',F0.6)
  INTEGER:: k

  DO k=SIZE(x),2,-1
    WRITE(efu,165) x(k),yu(k)
  END DO

  DO k=1,SIZE(x)
    WRITE(efu,165) x(k),yl(k)
  END DO
  RETURN
END SUBROUTINE PrintCSV

SUBROUTINE PrintXLS(efu, x, yu, yl)
  INTEGER,INTENT(IN):: efu
  REAL,INTENT(IN),DIMENSION(:):: x, yu, yl
  165 FORMAT(F0.6,'</Data></Cell><Cell><Data ss:Type="Number">',F0.6)
  INTEGER:: k

  DO k=SIZE(x),2,-1
    WRITE(efu, '(A)', advance='no') '<Row><Cell><Data ss:Type="Number">'
    WRITE(efu, 165, advance='no') x(k),yu(k)
    WRITE(efu, '(A)') '</Data></Cell></Row>'
  END DO

  DO k=1,SIZE(x)
    WRITE(efu, '(A)', advance='no') '<Row><Cell><Data ss:Type="Number">'
    WRITE(efu, 165, advance='no') x(k),yl(k)
    WRITE(efu, '(A)') '</Data></Cell></Row>'
  END DO
  RETURN
END SUBROUTINE PrintXLS

SUBROUTINE PrintSelig(efu, x, yu, yl)
  INTEGER,INTENT(IN):: efu
  REAL,INTENT(IN),DIMENSION(:):: x,yu,yl
  165 FORMAT(F10.6,'   ',F10.6)
  INTEGER:: k
